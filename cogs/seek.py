    ...
    async def seek(self, ctx, time):
        ...
        if len(main.bot.playing[ctx.guild.id]) != 0:
            ...
            if voice_channel is None:
                ...
                await msg.edit(content='You need to be in a voice channel to use this command.')
            else:
                ...
                await msg.edit(content=f'Now playing {main.bot.playing[ctx.guild.id][0]["title"]} from {formatted_time} seconds.')
        else:
            # send a message to play a song in order to seek in it
            await msg.edit(content='You need to play a song before you can seek in it.')
